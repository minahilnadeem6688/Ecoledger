...
export default function Login() {
  ...
  const form = (
    <View style={[s.card, { padding: isTablet ? S.xxl : S.xl }]}>
      ...
      {offline ? <Notice tone="red" icon="cloud-offline-outline">{API_ORIGIN.includes('localhost') || API_ORIGIN.includes('127.0.0.1')
            ? <>The EcoLedger server is not reachable. Start the backend with {'"npm start"'} and try again.</>
            : 'The server is waking up (free hosting sleeps when idle). This can take up to a minute; you can still sign in and it will wait.'}</Notice> : null}
