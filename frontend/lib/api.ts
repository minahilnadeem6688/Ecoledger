...
/** Proof photos are stored as "/api/proofs/<key>"; older ones were plain file names in /uploads. */
export const uploadUrl = (file?: string | null) => (!file ? null : file.startsWith('/') ? API_ORIGIN + file : `${API_ORIGIN}/uploads/${file}`);
...
// Generous default: a free-tier server can take up to a minute to wake up.
...
// On a public testnet a mint waits for a block, which can take up to half a minute.
const MINT_TIMEOUT = 90000;

/* ─── Endpoints ─────────────────────────────────────────────────────────── */

export const api = {
  health: () => request<Health>('/health', {}, 15000),
  ...
  verify: (id: string, status: 'approved' | 'rejected', reason?: string) =>
    post<{ activity: Activity; mint: MintResult | null }>(`/activity/${id}/verify`, { status, reason }, MINT_TIMEOUT),
  retryMint: (id: string) => post<{ activity: Activity; mint: MintResult }>(`/activity/${id}/retry-mint`, undefined, MINT_TIMEOUT),
...
/** Block explorer link for a transaction or address, when the chain has a public explorer. */
export const explorerLink = (chain: ChainStatus | null | undefined, kind: 'tx' | 'address', value?: string | null) =>
  chain?.explorerUrl && value ? `${chain.explorerUrl}/${kind}/${value}` : null;
