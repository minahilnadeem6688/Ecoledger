...
// DELETE /api/students/:id: remove a student and their data (admin), e.g. test accounts.
// Tokens already minted stay on-chain, as blockchain records are permanent.
router.delete('/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    const student = await Student.findById(req.params.id).catch(() => null);
    if (!student) return res.status(404).json({ error: 'Student not found.' });
    if (student.role === 'admin') return res.status(400).json({ error: 'Admin accounts cannot be removed here.' });
    await Promise.all([
      Activity.deleteMany({ studentId: student._id }),
      Proof.deleteMany({ owner: student._id }),
      Redemption.deleteMany({ studentId: student._id }),
    ]);
    await student.deleteOne();
    res.json({ message: `Removed ${student.email}.` });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
